...
## Full Walkthrough: One Team Workflow, Traced Step by Step (With Real Content)

Everything above can feel like five separate features until you watch them work together on **one real task**. The file already gives us the exact sequence under "The ShopAssist workflow, end to end" — this section just unpacks each of those six steps with the file's own real command, skill, and prompt content, so you can see *why* each tool gets picked, not just that it does.

The scenario: a developer has opened a pull request that touches ShopAssist's refund and escalation code.

---

### Step 1 — `/shopassist-review`

The developer starts the same way every teammate would: by running the shared command.
...
Running this produces a review with **file paths, a risk level, and suggested fixes** — exactly the output contract the command file specifies. Nothing here is improvised; the four focus areas (refund logic, escalation behavior, customer messaging, missing tests) are baked into the file, not something the developer has to remember to type.

That's the whole point of a **project-scoped command**: it lives in `.claude/commands/shopassist-review.md`, inside the repo, so it's shared through version control. Every developer on the team runs the *identical* review criteria — nobody is relying on memory for which four things matter, and nobody's review is more thorough than anyone else's by accident.

---

### Step 2 — Invoking the `support-workflow-analysis` skill

Say the review flags something in escalation behavior that looks off, but it's not obvious from a quick read — it needs someone to actually trace how refund eligibility and response generation interact. That's more than a one-line command can respectably do, so the developer reaches for the Skill instead.
...
Walking through what each field does for *this specific invocation*:

- **`name: support-workflow-analysis`** — this is the skill being called; it's what makes it discoverable and invokable.
- **`description`** — tells Claude Code when this skill is the right fit: refund, escalation, and response behavior, which is exactly what the flagged review comment is about.
- **`context: fork`** — this is the important one here. The investigation is going to mean reading a lot of files and comparing how refund eligibility, response generation, and test coverage all connect. That's noisy. `context: fork` runs the whole exploration in an **isolated context** — a separate branch of work — and only a **concise summary** comes back into the main conversation. The developer never has to scroll past dozens of file reads to find the answer.
- **`allowed-tools: Read, Grep`** — the skill can only *look*. No `Edit`, no `Write`. It can inspect the codebase and search it, but it cannot change a single line. That's the safety guarantee: an analysis skill that might get invoked casually can't accidentally rewrite refund logic while "just checking."
- **`argument-hint: "path to changed workflow or PR summary"`** — tells the developer exactly what to hand the skill: point it at the changed workflow, or paste in the PR summary from Step 1.

The skill runs, stays out of the way while it works, and hands back a summary — confirming there's a real gap worth fixing.

---

### Step 3 — Plan mode with an Explore subagent

The skill's summary reveals the fix isn't a one-liner: it touches refund eligibility logic, response generation, and test coverage across multiple files. That's a genuine multi-file, "there are several ways to do this" situation — exactly what Plan mode is for. Before touching any code, the developer wants a plan and a chance to approve it.

To gather the facts needed to write that plan without cluttering the conversation, the developer uses an Explore subagent with the file's own discovery prompt:
...
The subagent goes and reads the actual code — tracing dependencies, checking test files — and comes back with a **structured map**: refund logic location, response generation location, test coverage locations, and where coverage is missing. All of that discovery work — every file it opened, every dead end it ruled out — stays inside the subagent. The main conversation only sees the finished map, so it stays readable while a plan gets built on top of it.

With that map in hand, Plan mode produces a concrete, approvable plan for the refactor — separate decision paths for billing disputes, damaged items, and policy exceptions — and only after the developer approves it does any code get written.

---

### Step 4 — Direct execution (a separate, smaller fix)

While tracing the refund logic in Step 3, the developer notices something unrelated and much simpler: nothing stops a refund amount from going negative. This is not multi-file, not architectural, and there's exactly one obvious way to fix it — add a validation check. It doesn't deserve a planning phase at all.

So this one gets handled the opposite way from Step 3: no exploration, no plan, no approval step. The developer just tells Claude Code to add the check and it's implemented directly. Small, clear, one path — direct execution is the right tool precisely because there's nothing to design.

---

### Step 5 — Refinement

Tests run against the approved refactor from Step 3, and one fails. The wrong move here is telling Claude Code "it's still broken" — that's vague, and vague descriptions produce vague fixes. The right move is the test-driven-iteration pattern the file names directly: hand over the exact gap between what was expected and what actually happened.
...
That's the entire feedback. It says precisely which case is wrong (an escalation that should have triggered didn't) without making Claude Code guess what "broken" means or re-derive the failure from scratch. Claude Code can go straight to the escalation logic, compare it against this exact expectation, and fix the specific path that produced `false` instead of `true` — rather than re-reviewing everything that might conceivably be wrong.

---

### The whole workflow, end to end

1. `/shopassist-review` — shared command, same review criteria for every teammate, returns file paths + risk level + suggested fixes.
2. `support-workflow-analysis` skill — deeper, read-only investigation, run in an isolated fork so only the summary comes back.
3. Plan mode + Explore subagent — multi-file refactor gets mapped first, then planned and approved, before any code changes.
4. Direct execution — a small, obvious fix found along the way (negative refund amount) gets implemented immediately, no plan needed.
5. Tests run, one fails, and refinement closes the loop with the exact gap (`expected: escalation=true` / `actual: false`) instead of a vague complaint.

**The one thing to hold onto:** this one workflow used five different tools — a command, a skill, plan mode, direct execution, and refinement — and each one was picked for the specific kind of work it's actually good at. None of them is a universal hammer; the skill in using Claude Code well is routing each piece of work to the right one.
