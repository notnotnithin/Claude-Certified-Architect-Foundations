...
Everything above can feel abstract until you watch **one single message** go all the way through — schema in, real captured response out. So let's trace just one, start to finish, using the exact request and the exact output this lecture captured on screen. Nothing here is invented; it's all pulled straight from section 4 above.

The message we're tracing:

> "Can I return order 12345?"

---

### Step 1 — The tool schema Claude gets handed

Before this message ever arrives, ShopAssist has already defined one tool and described it to Claude as a strict contract:
...
This is `tools = [...]` — a list with exactly one tool in it. Claude will see this same schema on every request; the only thing that changes is the customer's message.

---

### Step 2 — The exact API call being sent

Here is the actual call, reused verbatim from section 4:
...
One tool, one user message, `max_tokens=500`. That's the whole request.

---

### Step 3 — The exact response Claude sent back

This is the **real, screenshot-captured output** — printed straight from `message.stop_reason` and `message.content`, unedited:
...
Two details worth stopping on, because they're easy to miss:

- **A `TextBlock` comes before the `ToolUseBlock`.** Claude said *"Let me look up that order for you right away!"* first, then requested the tool — in the same turn. Claude can narrate what it's about to do and request a tool at the same time; it's not one-or-the-other.
- **The `ToolUseBlock` carries `caller=DirectCaller(type='direct')`.** That field isn't discussed anywhere in the slide bullets or the transcript — it's an SDK-level detail that just happens to be visible in the real captured output, sitting right alongside the fields that *are* explained (`id`, `input`, `name`, `type`).

---

### Step 4 — What your backend code has to do to read this

No execution happens in this step — just reading what Claude asked for:

```python
if message.stop_reason == "tool_use":
    for block in message.content:
        if block.type == "tool_use":
            tool_name = block.name        # "lookup_order"
            tool_input = block.input      # {"order_id": "12345"}
```

Walked out loud:

1. Check `message.stop_reason == "tool_use"` — this is what tells you Claude wants a tool run, rather than just finishing with a normal reply (`end_turn`).
2. Loop through `message.content` — remember, it's a *list* of blocks, and here it holds two of them (the `TextBlock` and the `ToolUseBlock`).
3. Find the block where `block.type == "tool_use"` — skip past the `TextBlock`, that one's just Claude talking.
4. Pull `block.name` out of it — that's `"lookup_order"`, telling you *which* function to run.
5. Pull `block.input` out of it — that's `{"order_id": "12345"}`, the *arguments* to run it with.

That's it. At the end of Step 4, your code knows exactly which function Claude wants and exactly what to pass it.

---

### Where this lecture stops on purpose

This walkthrough ends here — deliberately. Nobody calls `lookup_order("12345")`. Nobody queries a database. Nobody sends a `tool_result` back to Claude, and Claude never sees a real order record or writes a final reply. Reading the request is as far as this file goes; running it and sending the answer back is the next lecture's job (lecture 20).

**The one thing to hold onto:** everything Claude did in this whole trace was produce well-formatted *text* describing what it wants — it never touched a database, and your code hasn't run anything yet either. Claude asked. That's all "tool use" means at this stage.
