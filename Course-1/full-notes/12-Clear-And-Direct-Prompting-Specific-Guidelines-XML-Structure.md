...
## Full Walkthrough: One Message, Traced Step by Step (With the Real Prompt and Output)

Everything above can feel abstract until you watch **one single customer message** go through the actual XML-tagged prompt and come back as a real reply. So let's trace just one message, start to finish, using this file's own screenshot-verified code and output. The message is the same one used throughout the lecture's Python example:

> "I want my money back."

---

### Step 1 — What the `prompt` variable actually equals

In the real code (Section 7 above), `customer_message` is set to `"I want my money back."` and dropped straight into the XML template through an f-string. Substitute it in, and here is exactly what the `prompt` variable equals — every tag filled in, all in one block, for the first time in this file:
...
This is the corrected version — it uses `<role>` where the source material's own hand-transcribed code mistakenly repeated `<rules>` instead (see the flagged correction in Section 7). It uses five of the seven tags from the full production anatomy in Section 4 — `<role>`, `<task>`, `<rules>`, `<customer_message>`, `<output_format>` — with the ShopAssist background folded straight into `<role>` and the "short, calm, asks for the order number" goal treated as an internal target rather than a separate tag. We'll check the real output against exactly that target in Step 3.

### Step 2 — Sending it as a single user message

This whole block isn't extra ceremony — it's just text. It goes to Claude as the content of one ordinary user message:
...
No special "XML mode," no schema, no tool. Claude reads the tagged text the same way it reads any other message — the tags are a convention Claude has learned to parse, not an API feature.

### Step 3 — The real output, checked line by line against the rules

Running that cell (`message.content[0].text`) produces this actual captured text:

> "Thank you for reaching out! I'm sorry to hear you're having an issue. I'd love to help look into this for you. Could you please provide your order number so I can review the details of your purchase? Once I have that information, I'll do my best to assist you with a suitable resolution. Thank you for your patience!"

Now hold that output up against every line of `<rules>`, one at a time:

- **Be polite and calm.** — Yes. "Thank you for reaching out," "I'm sorry to hear," "Thank you for your patience" — calm and courteous throughout.
- **Do not approve a refund.** — Yes. It never says the refund is approved and never promises one — it only offers to "look into this" and "assist... with a suitable resolution."
- **Ask for the order number.** — Yes. "Could you please provide your order number so I can review the details of your purchase?"
- **Keep the response under 80 words.** — Yes. The reply is roughly 55 words, comfortably under the limit.

Every rule holds, and the broader goal from Section 4's `<success_criteria>` — "short, calm, helpful — and asks for the order number" — checks out too: it's short, it's calm, it's helpful, and it asks for the order number.

### Step 4 — What the messy version would have risked

Compare that to what this file calls the messy approach — the same instructions as one run-on paragraph instead of tagged sections:
...
This isn't guaranteed to fail — Claude might still handle it fine today. The real risk, as Section 6 lays out, shows up over time: nothing here separates instruction from data from rules, so as more business rules and more customer data get added, the paragraph only gets harder to read and harder to safely edit. With the tagged version, changing the refund policy means editing just the text inside `<rules>`. With the messy paragraph, someone has to reread and re-thread the whole sentence to find the one clause that needs to change — and risks quietly breaking something else nearby while they're in there.

**The one thing to hold onto:** the XML tags didn't make Claude smarter — the same instructions, untagged, could have produced this same good reply. What the tags did was make those instructions unambiguous to both Claude, reading the prompt right now, and to the next developer who has to open this prompt six months from now and change one rule without breaking the other three.
