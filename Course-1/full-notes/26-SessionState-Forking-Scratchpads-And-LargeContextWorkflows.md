...
## Full Walkthrough: A Case Gets Interrupted, Then Resumed (With Real JSON)

Everything above can feel abstract until you watch **one real case** get knocked over mid-workflow and then picked back up. So let's follow `case_8421` — the jacket-refund case from earlier in this file — through an interruption and a resume, start to finish, with the actual data at every stage.

The whole idea, in plain words:

```
work stops (crash / timeout / anything) → nothing was kept in the model's head → resume_case() reloads two small JSON objects → work continues from the right spot
```

---

### The scenario — the case was mid-flight, then it just stopped

Picture this: someone (a human agent, or an earlier coordinator run) was in the middle of handling `case_8421`. The return policy had already been checked. The case was about to be escalated to a human, because the refund amount is over the automatic limit. Then — browser crash, session timeout, server restart, doesn't matter which — the process just stops.

If the only place that progress lived was inside a chat window, it's gone. Nobody knows the policy check already happened. Nobody knows escalation was next. The system is one bad guess away from re-running work it already did, or worse, silently dropping a step.

That's exactly the failure mode this lecture is built around. Here's why it doesn't happen in this design.

---

### Step 1 — The facts survived, because they were never *in* the conversation

Before the interruption, before any of this, the case's durable facts were already sitting outside the model context, in `case_facts`:
...
Notice what this dict does *not* depend on: it doesn't need the chat history to make sense. Read it cold, with zero prior context, and it already tells you everything that matters — who the customer is, what the order was, what they want ($149.00 refund, not a replacement), when it was delivered, and *why* this case needs a human (the requested refund is nearly double the $75.00 automatic limit). This is the "memory" that survived the crash, because it was never stored in memory in the first place — it was written to durable state.

---

### Step 2 — `resume_case("case_8421")` gets called, and does three loads
...
Walking through what each load actually hands back for `case_8421`:

**`load_case_facts("case_8421")`** returns exactly the dict above — the $149.00 refund request, the $75.00 limit, the delivery date, all of it. Nothing has to be re-derived or re-guessed from old messages.

**`load_recovery_manifest("case_8421")`** returns the case's recovery manifest:
...
This is the piece that actually tells the coordinator where the work stopped.

**`load_trimmed_case_events("case_8421")`** returns a short, trimmed slice of *recent, relevant* events only — not the entire case history. Think: the last policy-check result, the last customer message, maybe the photo-evidence note. Old back-and-forth, resolved side-questions, and verbose tool output from earlier in the case are left out on purpose — that's the same "reduce context pressure" idea from earlier in this file, applied at resume time instead of at message time.

---

### Step 3 — What this actually tells the coordinator

Put the manifest next to the facts and the next move is obvious, with no guesswork involved:

- `last_completed_step: "check_return_policy"` → the policy check already ran. Don't run it again.
- `pending_step: "escalate_to_human"` → the very next action is escalation, not "start over from figuring out what the customer wants."
- `last_tool_results.status: "requires_human_review"` → confirms *why* the next step is escalation: the policy check already determined this case needs a person, consistent with `case_facts["escalation_reason"]`.

A naive resume — one that just re-fed the whole chat transcript back into the model and asked "what should we do next?" — risks re-running the policy check from scratch, or worse, drifting into a different conclusion the second time around. The recovery manifest makes that unnecessary: the coordinator picks up exactly at `escalate_to_human`, with zero re-work.

---

### Step 4 — The trap: don't trust `safe_to_retry` blindly

Look again at the manifest's `safe_to_retry` list:
...
This is wrong, and it's wrong in a way worth walking through rather than quietly fixing. The lecture states its own rule plainly: *"Processing a refund is not safe to retry blindly."* Refunds are non-idempotent — if a refund already went through and the step gets blindly retried because a field in a data structure says it's "safe," the customer could be refunded twice. `draft_customer_response` belongs on this list (redrafting a message has no side effects). `process_refund` does not — its presence here contradicts the stated rule, and this file flags it above as a likely copy/paste error in the original course material rather than silently correcting it.

The lesson for the coordinator: a `safe_to_retry` list is a hint, not a law. It must still be checked against the actual rule — refund processing is never blindly retried, no matter what one field in one JSON object claims. That check belongs in code (a backend gate), not in trusting the manifest at face value. This is the same "prompts guide behavior, code enforces rules" principle from earlier in this file, just applied to the manifest itself instead of to the model's output.

---

### Step 5 — Spinning up a sub-agent for one piece of the resume

Before escalating, suppose the coordinator wants a quick, independent double-check: is $75.00 still the correct automatic refund limit under the current policy, or did `returns-v7` change since this case was opened? Rather than dragging the whole case (chat history, evidence notes, prior steps) into that question, the coordinator spins off a sub-agent with an isolated context — only the facts relevant to *this* narrow question:
...
sequenceDiagram
    ...
    Co->>S: Explicit handoff — ONLY:\npolicy_id="returns-v7",\nautomatic_refund_limit=75.00\n(not the full case history)
    S-->>Co: Structured finding:\n{"policy_id": "returns-v7",\n"current_limit": 75.00,\n"matches_case_facts": true}\n(reasoning transcript discarded)
```

The sub-agent never sees `customer_expectation`, `evidence`, the delivered date, or anything else it doesn't need for this one check. It comes back with a small structured finding — not a paragraph explaining its reasoning — and the coordinator folds that finding into the resume before proceeding to `escalate_to_human`. This is the same isolation pattern shown earlier in the file's sub-agent sequence diagram, just pointed at one concrete step of one concrete case.

---

### The whole journey, end to end (`case_8421`)

1. Case is mid-flight: policy already checked, escalation pending. Then the process is interrupted — for any reason.
2. Because durable state was never kept only in a chat window, nothing is lost. `case_facts` and `recovery_manifest` are sitting in storage, unaffected by the crash.
3. `resume_case("case_8421")` runs its three loads: `case_facts` (the full picture — $149.00 requested, $75.00 limit, damaged jacket), `recovery_manifest` (`last_completed_step: "check_return_policy"`, `pending_step: "escalate_to_human"`), and a trimmed slice of recent events (not the whole history).
4. The coordinator reads the manifest and knows, with no guessing: skip the policy check, go straight to escalation.
5. The coordinator does **not** blindly trust `safe_to_retry` — it knows `process_refund` is wrongly listed there and treats refund processing as never-safe-to-retry regardless.
6. A sub-agent gets spun up with only the two facts it needs (`policy_id`, `automatic_refund_limit`) to confirm the $75.00 threshold still holds, and reports back a small structured finding — not a transcript.
7. The coordinator escalates the case to a human, exactly where it left off.

**The one thing to hold onto:** everything the coordinator needed to resume this case correctly — what already happened, what's next, what's safe, what a sub-agent found — came from a couple of small JSON objects, not from re-reading a long conversation. That's the entire point of keeping state outside the model's context: the interruption barely matters, because the model's context was never the thing holding the case together.
