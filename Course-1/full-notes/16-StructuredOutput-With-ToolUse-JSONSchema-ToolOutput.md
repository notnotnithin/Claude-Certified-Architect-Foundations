...
Everything above can feel abstract until you watch **one single customer message** travel through this lecture's version of the pattern. So let's follow the same messy message this lecture uses everywhere else, start to finish, with the real captured JSON at every stage.

> "I got my shoes yesterday and they're scratched. I don't know if I can return them, but I want a replacement. I can send a photo if needed."

The whole thing here is short — just two moves:

```
message in → Claude fills a form → your code reads the filled form
```

That's it. (Lecture 19 adds validation, retries, and human review on top of this — none of that happens yet in this lecture.)

---

### Step 1 — The blank form

Before this message ever arrives, the tool's `input_schema` is already defined — the same `extract_return_request` schema shown earlier in this file:

```json
{
  "order_id": ["string", "null"],
  "item": ["string", "null"],
  "reason": "enum: normal_return | damaged_item | billing_dispute | policy_exception | unclear | other",
  "reason_detail": ["string", "null"]
}
```

Two choices matter here, same as before: `order_id` and `item` can be `null`, so Claude never has to invent a value just to fill the box. `reason` is a fixed list, so "scratched" has to land on one of *your* labels, not whatever wording Claude feels like using.

---

### Step 2 — Send the message + the form, forced

One API call, carrying both the schema and the message, with `tool_choice` pinned so Claude has no option to just chat back:
...
`customer_message` here is the "shoes... scratched" message above. Nothing gets executed — Claude just reads the sentence and fills in the tool's `input`.

---

### Step 3 — Pull the filled form straight out

No parsing, no second call. The code just finds the `tool_use` block and reads its `input`:
...
### Step 4 — The real output, field by field

This is the actual captured result — a plain Python dict, not a JSON string:
...
- **`order_id: None`** — the customer never gave one, so Claude left it `null` instead of inventing an order number.
- **`item: 'shoes'`** — read straight off the message, no guesswork needed.
- **`reason: 'damaged_item'`** — "scratched" doesn't literally appear in the enum, but the normalization rule ("use `damaged_item` when the item arrived... scratched...") tells Claude to fold it into that fixed category.
- **`desired_action: 'replacement'`** — the customer explicitly asked for a replacement, not a refund, and Claude kept that distinction rather than defaulting to something generic.
- **`evidence_provided: False`** — the customer only offered to send a photo *if needed*, not that they already have one attached, so this correctly reads as "not yet provided."
- **`missing_information: ['order_id']`** — the gap isn't silently dropped. It's logged explicitly, so downstream code (or a human) knows exactly what's still missing.

---

### Where it stops

This is as far as this lecture's pipeline goes. There's no validation step here, no retry, no routing decision — the code just now *has* this dict. What happens next (checking whether `order_id` being missing should block anything, deciding if this needs a human, deciding what to automate) isn't built in this lecture at all — that's lecture 19, which wraps exactly this same pattern in a full validate → retry → route pipeline.

**The one thing to hold onto:** the nullable `order_id` and the fixed `reason` enum aren't decoration — they're doing the actual work in this example. They're the reason Claude reported an honest gap (`order_id: None`, logged in `missing_information`) instead of guessing a fake order number, and the reason "scratched" became the clean, reusable label `damaged_item` instead of a one-off phrase like "scratch damage."
