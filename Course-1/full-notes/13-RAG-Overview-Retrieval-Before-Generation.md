...
## Full Walkthrough: One Question, Traced Step by Step (With Real Values)

The three-stage pipeline (Prepare → Retrieve → Generate) can feel abstract until you watch **one real question** move through it. So let's trace the file's own worked example, start to finish, using the actual chunk names and the actual embedding numbers already given above — not invented ones.

The question we'll follow:

> "Can I return a damaged item after 45 days?"

---

### Step 1 — Prepare (already done, before this question ever exists)

This step doesn't happen per-question — it happened once, ahead of time. ShopAssist's policy documents (return policies, exception policies, escalation procedures, and so on) were already split into chunks, converted to vectors, and saved in a vector database, long before this customer typed anything. `docs → chunks → store`. By the time a question arrives, there's nothing left to prepare — only to search.

---

### Step 2 — Retrieve: turn the question into numbers, then find the closest chunks

When the question arrives, it gets converted into its own embedding vector, the same way every stored chunk already was. Then the system compares that question-vector against every chunk-vector in the store and keeps whichever are numerically closest.

The file's own embedding example shows exactly this mechanic — reusing it here only to illustrate *how* closeness works, not as the literal vector for this exact question:

```
"refund eligibility"        → [0.82, 0.10, 0.93, ...]
"Can I get my money back?"  → [0.79, 0.14, 0.90, ...]   ← close to "refund eligibility"
"shipping delays"           → [0.06, 0.88, 0.12, ...]   ← far from "refund eligibility"
```

The same pattern applies to "Can I return a damaged item after 45 days?": a stored chunk about damaged-item return policy would sit numerically close to the question's vector — small differences across the same dimensions, the way `[0.79, 0.14, 0.90]` sits close to `[0.82, 0.10, 0.93]` above. A chunk about something unrelated, like shipping delays, would sit numerically far away, the way `[0.06, 0.88, 0.12]` does. The vector database's whole job is finding the "close" chunks and ignoring the "far" ones.

For this question, that search returns the file's own three named chunks:

- **Damaged-item return policy**
- **Exception policy**
- **Escalation instructions**

That's the entire output of Retrieve: `question → top-k chunks`. Nothing has been answered yet — just found.

---

### Step 3 — Generate: the retrieved chunks and the question become one real prompt

The three chunks and the original question now get placed into a single prompt sent to Claude — chunks first, question last, exactly as the pipeline diagram shows: `chunks + Q → answer`. Here's what that prompt would look like as an actual text block (the chunk *wording* below is illustrative — the file only names the chunks by category, not their literal text — but the structure is faithful to the pipeline):

```
Use the following policy excerpts to answer the customer's question.

[Damaged-item return policy]
"Items damaged in shipping or found defective on arrival may be
returned for a full refund or replacement..."

[Exception policy]
"Return windows may be extended beyond the standard period when
the item was damaged, defective, or the delay was not the
customer's fault..."

[Escalation instructions]
"If a return falls outside standard policy but qualifies under an
exception, escalate to a support agent for manual approval before
processing..."

Question: Can I return a damaged item after 45 days?
```

Claude reads only what's in this prompt and writes an answer grounded in those three excerpts — something like *"Yes — damaged items can qualify for an exception to the standard return window, but this case should be escalated for manual approval."* (illustrative wording; the file doesn't record the literal generated answer, only that Claude generates a response from these three retrieved excerpts.)

---

### The core warning, restated at this exact point

Claude is **not** answering from memory here. It never "learned" ShopAssist's return policy — it is only looking at the three chunks Step 2 handed it. If Step 2 had retrieved the wrong chunks — say, a shipping-delay policy and a billing FAQ instead — Step 3 would still produce a fluent, confident-sounding answer. It would just be confidently wrong, and no amount of model quality in Step 3 could fix a mistake made in Step 2.

---

### The whole journey, end to end (this question)

1. **Prepare** (already done): ShopAssist's policies are chunked, embedded, and stored ahead of time.
2. **Retrieve**: "Can I return a damaged item after 45 days?" is embedded, compared against stored chunk vectors, and the three closest chunks come back — damaged-item return policy, exception policy, escalation instructions.
3. **Generate**: those three chunks plus the question form one prompt; Claude answers using only that evidence.

**The one thing to hold onto:** RAG succeeds or fails at the **Retrieve** step. Generation just faithfully reports whatever it was handed — it can't tell the difference between the right evidence and the wrong evidence, so all the real risk (and all the real leverage) lives in what gets retrieved.
