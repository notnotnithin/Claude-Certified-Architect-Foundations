...
## Full Walkthrough: One Task, Traced Through Its Whole Lifecycle

Everything above can feel like a list of separate commands until you watch **one real task** move through all of them in order. So let's follow the file's own recurring example — the return-validation work — from the day it starts to the day it gets implemented.

The whole lifecycle is this loop, in plain words:

```
name it → let it grow → compact it → back it up → resume it tomorrow → fork the hard question → compare → implement the winner
```

---

### Day 1 — start and name the session

The developer opens Claude Code and names the session right away, instead of leaving it as an anonymous ID:

```text
claude -n return-validation
```

This is the file's own point about naming: giving a session a readable identity up front is what makes it **findable later** — instead of hunting through a picker or an unlabeled session ID next week, `claude --resume return-validation` will just work.

---

### The session grows large

Work continues for a while — reading `app/returns/validation.py`, tracing `validate_return_request()`, running the test suite, trying a few things that don't pan out. By the end of the day, the session shows the exact signs the file warns about:

- Multiple conflicting ideas got explored along the way
- Several unrelated files got opened while chasing the bug
- Long test-log output got pasted straight into the chat

The session isn't broken — it's just noisy. This is the point where **compact** earns its keep.

---

### Compact it

Running `/compact` shrinks the conversation down to a summary. The file gives an exact example of what a *good* one looks like:
...
Notice what this summary does **not** do: it doesn't say "we changed some validation stuff." It keeps the exact function name (`validate_return_request()`), the exact failing test name (`test_damaged_item_requires_photo`), the exact expected-vs-actual values (`"needs_review"` vs. `"approved"`), and the exact next step (`Edit validation.py, run pytest.`). Anyone — or any future session — reading this summary knows precisely where to pick up, with nothing left to guess at.

---

### Save a scratchpad too

Compacting isn't the only backup. The developer also writes the same key facts into a scratchpad file, in the exact form shown on the slide:
...
Why keep both a compact summary *and* a scratchpad? Because they protect against different failure modes. A compact summary still lives **inside** the session — it survives a bloated context window, but not the loss of the session itself. A scratchpad is a plain file in the project, so it survives even if the whole session or chat history is lost, not just compacted.

---

### Next day — resume

The next morning, the developer picks the task back up by name:

```text
claude --resume return-validation
```

But before touching anything, the file's golden rule applies: **resume the conversation, but verify the code.** The repo may have moved on overnight — someone else could have pushed a commit, a dependency could have changed, a test could now be passing or failing for a different reason. So the first move isn't to keep editing from the compacted summary — it's `git status`, `git diff`, and re-running the relevant tests, to confirm the stale mental model in the transcript still matches reality.

---

### A design question comes up — fork

Reading the code again raises a real question: is the smallest possible fix enough, or does this deserve a proper redesign? Rather than debate that back and forth inside the main session — and fill it with exploration that may not go anywhere — the developer forks it into two isolated explorations:
...
Both forks are given the same restriction on purpose: **do not edit files.** Each one only investigates and reports back — which files it would touch, what risks it sees, what tests should run. Because neither fork can actually change anything, running both costs nothing beyond the time to read their findings, and the main session never gets polluted with two competing, half-finished implementations.

---

### Compare and implement

The two reports come back. Fork A's findings are small and contained — a change scoped entirely inside the existing function. Fork B's findings describe a bigger, more invasive redesign. The developer compares the two and picks Fork A, the smaller change, then implements **only that one**, in the main session. Fork B's exploration — the `ReturnPolicyValidator` design — is simply discarded; it did its job by existing as a comparison, not by being built.

---

**The one thing to hold onto:** naming the session, compacting it well, writing a scratchpad, and forking the hard question are all answering the same one question — if you had to walk away right now and someone else (or a future you) picked this up tomorrow, would they have what they need? Durable facts — the exact function, the exact test, the exact error, the exact decision — have to survive even when the session itself doesn't.
