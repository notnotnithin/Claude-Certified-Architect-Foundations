...
The priority-order idea (Section 4) can feel abstract until you watch **one single message** actually walk through it. So let's trace just one, start to finish, using the file's own real overlap example — the one already introduced above:

> "My package arrived damaged, and I was charged twice."

---

### Step 1 — The exact prompt Claude receives

Nothing new gets invented here — this is the same classification prompt from Section 4, with this specific message dropped into the customer-message slot:
...
That's it — one prompt, one message, one API call. Nothing runs yet; Claude just reads the message against the categories above.

---

### Step 2 — Why this one message genuinely fits two categories

Before Claude can pick, it's worth seeing *why* this is actually ambiguous — not a trick, a real overlap:

- **"My package arrived damaged"** — this is a textbook match for `damaged_item`: "Item arrived broken, defective, missing parts, or not working."
- **"and I was charged twice"** — this is a textbook match for `billing_dispute`: "Incorrect, duplicate, or unauthorized charge, missing refund, or payment problem."

Both readings are *correct*. Neither is a misread of the sentence. If Claude only had category definitions and no tie-breaker, it could reasonably land on either one — and, worse, land on a different one next time the same sentence comes through. This is exactly the boundary problem from Section 2, just showing up between two well-defined categories instead of one vague word.

---

### Step 3 — The priority list breaks the tie (and stops immediately)

This is the part that's easy to skim past: Claude doesn't weigh both matches and pick the "better" one. It walks the priority list from the top and **stops at the first match**:

```
1. escalation_candidate?  → No trigger present (no manager request, no legal
                             threat, no mention of the bank, no safety issue,
                             no extreme-frustration + billing/legal/policy).
                             SKIP.

2. billing_dispute?       → Yes — "charged twice" is a duplicate charge.
                             MATCH. STOP HERE.

3. damaged_item?          → (never checked — the list already stopped at #2)
4. policy_exception?      → (never checked)
5. normal_return?         → (never checked)
```

That last part matters: `damaged_item` is also a real match for this message, but the priority list never even gets there. It's not that `billing_dispute` was judged to "fit better" — it's simply higher on the list, and the check stops the moment it finds a hit. That's the whole mechanism from Section 4's priority order, just walked one line at a time.

---

### Step 4 — The resulting JSON (illustrative)

Given that trace, here's a plausible response for this message. The exact `reason` string isn't shown verbatim in the file's screenshots — this is a constructed example consistent with the file's own explanation, not a captured output:

```json
{
  "category": "billing_dispute",
  "reason": "Customer reports a duplicate charge, which ranks above damaged_item in the priority order even though the item also arrived damaged."
}
```

Notice the `reason` field does real work here — it doesn't just restate the category, it names the *other* category that was also in play and explains why it lost. That's a good sign the priority rule was actually applied, not guessed around.

---

### Step 5 — Same priority list, different outcome: adding an escalation trigger

To see the priority list do its job a second way, take the file's own escalation few-shot example: **"Charged twice… I'll dispute with my bank."** → `escalation_candidate`. Walk it through the same list:

```
1. escalation_candidate?  → Yes — "I'll dispute with my bank" is an explicit
                             escalation trigger ("contacting the bank /
                             disputing charge").
                             MATCH. STOP HERE.

2. billing_dispute?       → (never checked — even though "charged twice" is
                             right there and would have matched too)
3. damaged_item?
4. policy_exception?
5. normal_return?
```

Same list, same top-down process — but this time the very first rule fires, so the check stops one step earlier and lands on `escalation_candidate` instead. Nothing about how the list works changed; only which trigger showed up in the message changed.

---

### The whole journey, end to end (our damaged + charged-twice example)

1. Customer message arrives: "My package arrived damaged, and I was charged twice."
2. The full classification prompt (categories + escalation triggers + priority order + few-shot examples) is sent with this message in the customer-message slot.
3. Claude checks it against both plausible categories — `damaged_item` (arrived damaged) and `billing_dispute` (charged twice) — and confirms it's a genuine overlap, not a misread.
4. Claude walks the priority list top to bottom: `escalation_candidate` — no trigger, skip. `billing_dispute` — matches, **stop**.
5. Claude never evaluates `damaged_item`, `policy_exception`, or `normal_return` — the list already resolved at step 2.
6. Output: `{"category": "billing_dispute", "reason": "..."}` (illustrative JSON above) — the same result every single time this exact message is classified.

**The one thing to hold onto:** without the priority order, "arrived damaged, and I was charged twice" could get classified as `damaged_item` on one run and `billing_dispute` on the next — both are defensible reads of the sentence. The priority list is what removes that coin flip: it doesn't pick the "best" category, it just always checks in the same order and stops at the first match, so the same message lands the same way every time.
