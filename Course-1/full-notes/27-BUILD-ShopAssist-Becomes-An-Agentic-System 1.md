...
Everything above can feel abstract until you watch **one single customer message** travel through the whole agentic loop. So let's follow the file's own real example — the ORD-77819 case — start to finish, turn by turn, with the actual tool calls and actual results at every stage. No new scenario here, just the same case unpacked into a continuous trace.

> "Hi, I received my blue jacket yesterday and it arrived damaged. I also think I was charged twice. I want a full refund, not a replacement. My order number is ORD-77819."

Notice this one message bundles **three separate concerns** at once: a damaged item, a refund request (explicitly *not* a replacement), and a possible duplicate charge. Nothing forces Claude to handle these in a fixed order — it has to figure out, turn by turn, what to check next based on what it just learned.

---

### Turn 1 — Claude asks: "is this customer even who they say they are?"

Before anything else, Claude requests the one tool that gates every sensitive action downstream:

**Claude requests:**
```json
{ "type": "tool_use", "name": "verify_customer", "input": {} }
```

Notice the empty `input` — `verify_customer` takes no arguments from Claude at all. It checks a session the backend already holds (`SESSION["session_token"]`), not anything the model supplies.
...
That `customer_id` — `CUS-1842` — gets written into `case_facts`. It's now a verified fact the rest of the loop can rely on, not something Claude has to remember or re-derive later.

---

### Turn 2 — Claude asks: "what does this order actually look like?"

With a verified customer in hand, Claude requests the order lookup, passing the customer ID it just received plus the order number the customer typed in their message:
...
```json
{
  "type": "tool_use",
  "name": "lookup_order",
  "input": { "customer_id": "CUS-1842", "order_id": "ORD-77819" }
}
```

**Backend returns:** the order details for CUS-1842 / ORD-77819 — the only combination the mock backend recognizes as valid. That result, too, gets folded into `case_facts`.

---

### Turn 3 — Claude asks two independent questions at once

Now Claude has enough context to chase both remaining concerns — and here's the interesting part: **it requests both tools in the same turn, in parallel**, because the return-policy check and the duplicate-charge investigation don't depend on each other. Neither answer changes what the other tool needs to do its job.

**Claude requests (same turn):**
```json
{ "type": "tool_use", "name": "check_return_policy", "input": { "customer_id": "CUS-1842", "order_id": "ORD-77819", "item_id": "...", "reason": "damaged_item" } }
```
```json
{ "type": "tool_use", "name": "investigate_duplicate_charge", "input": { "customer_id": "CUS-1842", "order_id": "ORD-77819" } }
```

**Backend returns (policy check):**
```json
{
  "status": "success",
  "policy_id": "returns-v7",
...
**Backend returns (duplicate charge investigation):** no second charge was actually captured — there's a **pending authorization** on the account that was never billed, not a real duplicate charge. The customer's suspicion was reasonable, but the facts say it isn't a second charge.

So far: the item is eligible for a refund, but there's a hard ceiling of **$75.00** on what can be approved automatically. The customer asked for a **full refund** — remember, they explicitly said "not a replacement." That's where Turn 4 runs into trouble.

---

### Turn 4 — Claude requests the refund, and the tool itself says no

Claude now requests the refund, using the case's actual numbers:
...
```json
{
  "type": "tool_use",
  "name": "process_refund",
  "input": { "customer_id": "CUS-1842", "order_id": "ORD-77819", "refund_amount": 149.00, "reason": "damaged_item" }
}
```
...
$149.00 is well above the $75.00 automatic limit the policy check surfaced back in Turn 3. Read that response carefully: it isn't Claude deciding "this refund is too big, I shouldn't approve it." Claude never had the authority to approve it in the first place — it just called a tool. It's the **backend hook** inside `process_refund` that did the actual blocking, and it even hands back a `next_step` telling the agent exactly what to do about it. That's the whole point of the design: the tool itself is what stopped an over-limit refund, not Claude's own judgment. If Claude had somehow "decided" $149 was fine and skipped this call, nothing downstream would have stopped it — the enforcement lives in the code, not the prompt.

---

### Turn 5 — Claude escalates, with the full case attached

Because `process_refund` explicitly returned `next_step: "escalate_to_human"`, Claude now requests the escalation tool — not a vague "something's wrong," but a structured handoff carrying everything gathered so far: the verified customer and order, the root cause (damaged item + refund over limit), the requested amount, the duplicate-charge finding, and what's still missing.
...
```json
{
  ...
  "input": {
    "customer_id": "CUS-1842",
    "order_id": "ORD-77819",
    "root_cause": "Damaged item, refund requested exceeds automatic approval limit ($149.00 > $75.00)",
    "refund_amount": 149.00,
    "recommended_action": "Manual refund approval",
    "evidence": ["damage claim logged", "pending authorization, not a duplicate charge"],
    "missing_information": [],
...
**Backend returns:** a handoff ticket, `#HANDOFF-3087`, created and ready for a human specialist to pick up.

---

### Final step — the customer-facing reply, built from case_facts, not memory

Only now — refund blocked, case escalated — does the loop reach a real stopping point (`stop_reason: "end_turn"`), and Claude drafts its reply. The instructions explicitly told it not to produce a final response until either the refund succeeded or the case was escalated — and this is the escalated branch.

The screenshot-verified draft, reused verbatim from earlier in this file:
...
Every fact in that table — the order number, the delivery date, the eligibility, the pending-not-duplicate charge, the $149.00 figure, the `HANDOFF-3087` reference — came straight out of `case_facts`, the memory the loop built up turn by turn. Claude isn't reconstructing the conversation from what it remembers saying earlier; it's reading back verified state that was written down as each tool result came in. That's why the reply can be trusted: it's built from facts, not from recollection.

---

### The whole journey, end to end (our ORD-77819 example)

1. Customer message arrives, bundling three concerns at once.
2. **Turn 1:** `verify_customer()` → `customer_id: "CUS-1842"`.
3. **Turn 2:** `lookup_order("CUS-1842", "ORD-77819")` → order details retrieved.
4. **Turn 3 (parallel):** `check_return_policy(...)` → `automatic_refund_limit: 75.00`; `investigate_duplicate_charge(...)` → pending authorization, not a real second charge.
5. **Turn 4:** `process_refund(refund_amount: 149.00)` → `status: "blocked"`, `error_code: "REFUND_LIMIT_EXCEEDED"`, `next_step: "escalate_to_human"`.
6. **Turn 5:** `escalate_to_human(...)` → handoff `#HANDOFF-3087` created.
7. **Final step:** Claude drafts the customer reply from `case_facts` — only after the loop hit a real stopping point.

**The one thing to hold onto:** every single one of those five tool calls happened because Claude decided, in the moment, based on what the *previous* result told it — verify, then look up, then check policy and duplicate charge together once it knew they didn't depend on each other, then request the refund, then escalate once the refund tool itself said no. Nothing here was a hard-coded five-step script; it was a loop reacting to real results, one turn at a time.
