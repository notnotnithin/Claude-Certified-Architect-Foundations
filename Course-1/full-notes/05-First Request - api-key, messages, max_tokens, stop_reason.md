...
Everything above can feel abstract until you watch **one single message** go out to Claude and one single reply come back, with the real JSON at every step. This is the simplest possible version of that trip — no tools, no system prompt, nothing fancy. Just: send a question, get a text answer back. Almost every later lecture in this course is a fancier version of this exact same trip.

The one message we're tracing:

> "A customer wants to return an order. Write a short helpful response."

---

### Step 1 — What you send

Your code builds one plain JSON-shaped request and hands it to `client.messages.create()`:
...
Three things are in that request, and that's genuinely it:
- `model` — which Claude model should answer
- `max_tokens` — a ceiling on how long the reply is *allowed* to get (300 here — not a target, just a limit)
- `messages` — a list with one entry: who's talking (`"role": "user"`) and what they said (`"content"`)

---

### Step 2 — What comes back

Claude reads the one message, writes a reply, and the SDK hands your code back a `message` object. Simplified down to the parts that matter, it looks like this:
...
```json
{
  "content": [
    {
      "type": "text",
      "text": "Hi there,\n\nI'm sorry to hear you'd like to return your order! I'm happy to help make this process as easy as possible. Here's what to do next:\n\n1. Have your order number ready (found in your confirmation email)\n2. Let us know the reason for the return (damaged, wrong item, changed your mind, etc.)\n3. Check your eligibility — most items can be returned within 30 days of delivery\n\nOnce we confirm your return, we'll send you a prepaid return label via email...\n\nNeed help now? Email support@example.com or use Live Chat, Mon–Fri 9am–5pm."
    }
  ],
  "stop_reason": "end_turn",
  "stop_sequence": null,
...
That `text` field is the real reply this run actually produced (the full version, with all three numbered steps and the contact options, is up in [Section 6](#6-inspecting-the-api-response) above). And `output_tokens: 224` is a real, screenshot-captured number — this exact reply cost 224 output tokens to generate, comfortably under the 300 ceiling we set.

Nothing was "run" and nothing was looked up. Claude just read the one sentence and typed a reply. That's the whole job here.

---

### Step 3 — Your code pulls two things out of that response

There are only two lines your code actually needs to write:
...
**First, the text.** `message.content` is a list of content blocks — for a plain text reply like this one, there's exactly one block, and its `.text` is the reply itself:

```python
reply = message.content[0].text
```

**Second — and check this *before* you show the reply to anyone** — `message.stop_reason`. It tells you *why* Claude stopped writing:

```python
reason = message.stop_reason   # "end_turn"
```

Here's the same table from Section 6, because this is the exact decision your code has to make with that value:
...
In this run, `stop_reason` came back `"end_turn"` — Claude finished its own thought naturally, it wasn't cut off. So the branch your code takes is simple: **just print the text, it's complete and safe to show the customer as-is.**

Now picture the same request, but imagine `stop_reason` had come back `"max_tokens"` instead. Nothing about *how you read the response* would change — `message.content[0].text` would still work — but the table says that text might stop mid-sentence, mid-list, even mid-word, because Claude was forced to stop before it was done. You would not want to hand that straight to a customer without at least a check, or a retry with a higher `max_tokens`. That's the entire reason this field gets checked first, every time, rather than trusting the text blindly.

---

### The whole trip, end to end

1. Customer's need gets written as one plain-text prompt.
2. Your code sends `{model, max_tokens: 300, messages: [{role: "user", content: "..."}]}` in one call to `client.messages.create()`.
3. Claude replies once — no tools, no back-and-forth — and the SDK hands back a `message` object.
4. Your code reads `message.stop_reason` first: it's `"end_turn"`, so the reply is complete.
5. Only then does your code read `message.content[0].text` and show it to the user.
6. Along the way, `message.usage.output_tokens` (224, here) is sitting there too — not needed to display the answer, just useful for knowing how much the reply actually cost against the 300-token ceiling.

**The one thing to hold onto:** this exact shape — send `model` + `max_tokens` + `messages`, read `.content[0].text`, always check `.stop_reason` first — is the shape of nearly every Claude API call in the rest of this course. Later lectures bolt tools, system prompts, and multi-turn conversations onto it, but this same skeleton is underneath all of it.
