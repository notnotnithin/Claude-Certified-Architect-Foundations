...
Everything above can feel abstract until you watch **one single customer message** travel through the coordinator and out to a subagent and back. So let's follow the exact message this lecture uses everywhere else, start to finish, with the real JSON at every stage:

> "I think I was charged twice for order A1042. Also, the headphones arrived damaged, and I want to know if I can still return the charger from my previous order?"

---

### Step 1 — One message, three separate problems

Before anything gets delegated, notice what's actually inside this sentence. It isn't one issue wearing three sentences — it's **three unrelated issues** stapled together in one message:

1. A **billing issue** — a possible duplicate charge on order A1042.
2. A **damaged item** — the headphones arrived broken (an order investigation).
3. A **policy question** — can a charger from a *different, earlier* order still be returned.

None of these three share evidence. The billing question needs payment records. The damage question needs shipment records. The policy question needs the return-window rules. That's exactly the shape of problem this lecture says a single overloaded agent handles badly — and a coordinator with specialist subagents handles well.

---

### Step 2 — The coordinator needs `Task` in its own toolbox first

Before the coordinator can delegate any of this, it has to actually be *allowed* to spawn subagents:
...
`Task` sitting inside `allowedTools` is not a formality — it's the one thing that turns "the model thinks it should delegate this" into "the model can actually delegate this." Drop `Task` from that list and the coordinator can still *reason* its way to "this needs a billing specialist," but it has no tool call that would ever spawn one. The idea would just dead-end in its own head.

---

### Step 3 — The wrong assumption: a vague `Task` call

Say the coordinator decides to hand the billing piece off. The lazy version looks like this:
...
Read that prompt as if you were the `billing_analysis_agent` receiving it, with **zero memory of this conversation**. You'd have no customer id — so no way to even pull the right account's payment events. No verification status — so no way to know if you're even allowed to discuss this account's charges. No mention of order A1042, or that the concern is a *duplicate* charge specifically, or the customer's own words describing it. "Check the billing problem" isn't an instruction — it's a placeholder for one. The subagent would have nothing to actually investigate.

---

### Step 4 — The correct version: everything spelled out by hand

Here's what the coordinator actually sends, real JSON from this lecture:
...
- **`Customer context: customer id C-8821 - verified: true`** — the subagent doesn't have to go figure out who it's even talking about, or whether it's safe to discuss this account's billing at all. That question is already answered before the subagent does a single thing.
- **`Original message: "...charged twice for A1042..."`** — the subagent gets the customer's actual words as evidence, not a summary the coordinator might have gotten wrong. It can reason from what the customer really said.
- **`Relevant issue: possible duplicate charge for A1042`** — this narrows a whole conversation down to the one slice that's this subagent's job, filtering out the damaged-headphones and old-charger threads entirely.
- **`Return: {issue, evidence, conclusion, recommended_next_step, missing_information}`** — this tells the subagent exactly what shape to hand back. Without it, the subagent might reply in prose, or with the wrong fields, and the coordinator would have nothing reliable to stitch together later.

Every one of those lines exists because the subagent's context started completely blank. Nothing on that list was "obvious" to the subagent — it was obvious only to the coordinator, which is why the coordinator had to type it out.

---

### Step 5 — What comes back: the finding, not the thinking

The `billing_analysis_agent` goes off, uses its own narrow toolset (`get_payment_events`, `get_refund_status`), and reports back:

```json
"billing": {
  "conclusion": "two authorizations, only one captured",
  "next": "explain the 2nd auth drops off automatically"
}
```

That's it. However the subagent got there — which tool calls it made, in what order, what it ruled out along the way — none of that travels back to the coordinator. Only the structured conclusion crosses the boundary. The coordinator never sees the subagent's reasoning trace, only its answer. That's the same one-way shape as Step 2 of the ShopAssist extraction pipeline: the coordinator asks a narrow question and gets a narrow, structured answer — not a transcript of how the answer was produced.

---

### Step 6 — The same pattern, twice more, and possibly all at once

The damaged-headphones concern and the old-charger policy question go through the identical mechanic: a hand-built `Task` prompt with customer id, evidence, and a required return shape, sent to `order_investigation_agent` and `policy_review_agent` respectively. Their findings land in the same `subagent_findings` object:
...
Because the billing investigation, the order investigation, and the policy check don't depend on each other's answers — nothing about the duplicate-charge question needs the return-window ruling first — the coordinator doesn't have to run them one after another. It can emit all three `Task` calls in the same response and let them proceed in parallel, then wait for all three findings before writing the one reply the customer actually sees.

---

### The whole journey, end to end (our real example)

1. Customer sends one message containing three unrelated problems.
2. Coordinator's `allowedTools` includes `Task`, so it's actually able to delegate rather than just reason about delegating.
3. For each concern, the coordinator writes a `Task` prompt by hand — customer id, verified flag, the customer's original words, the specific issue, and the required return shape — because the subagent starts with none of that.
4. A vague prompt like `"Check the billing problem."` would have left the subagent with nothing to work from; the real `billing_task` prompt gives it everything it needs instead.
5. Each subagent runs with its own narrow toolset and reports back only a structured finding (`conclusion`, `next`) — not its reasoning process.
6. Since the three investigations are independent, the coordinator can fire all three `Task` calls together instead of waiting on each in turn.
7. All three findings route back through the coordinator, which is the only place they get combined into the single reply the customer receives.

**The one thing to hold onto:** the subagent never saw the original conversation — everything it knew came from the words the coordinator chose to type into that one `prompt` string.
