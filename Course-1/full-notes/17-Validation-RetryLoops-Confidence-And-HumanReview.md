...
## Full Walkthrough: The Exchange Retry, Traced Step by Step

Everything above can feel abstract until you watch **one single validation failure** get caught, fixed, and then checked all over again — with the lecture's own real values, not a made-up example. This lecture already gives us one: Claude returns `desired_action: "exchange"`, a word that isn't on the approved list. Let's follow that exact case from the first bad answer to the final routing decision.

```
Claude extracts → schema validation checks the shape → fixable? retry : leave it → re-check → route
```
...
Claude extracts a return request from a customer message and returns:

```json
{ "desired_action": "exchange" }
```

One field, one word — and it's already wrong in a very specific way. Not garbage, not blank. Just a word that isn't one of the four values this schema allows.

---

### Step 2 — Schema validation catches it

`validate_return_request` runs exactly the way this lecture's code defines it:
...
- `extraction["desired_action"]` is `"exchange"`.
- `"exchange" not in ("refund", "replacement", "store_credit", "unclear")` evaluates to `True`.
- So `errors.append("desired_action invalid")` runs, and `errors` is no longer empty.

That's the entire check. No judgment, no AI — just a Python `in` test deciding that `"exchange"` isn't one of four allowed strings.

---

### Step 3 — Decide: is this fixable?

Before anything gets resent to Claude, the lecture's golden rule gets applied: *retry when the model made a fixable extraction mistake; do not retry when the source simply does not contain the answer.*

So — does the original message actually contain a desired action? Yes. The customer clearly asked for *some* kind of resolution — Claude read that correctly enough to produce a confident-sounding answer. It just reached for a word, "exchange," that was never on the four-item list this schema allows. The information isn't missing; it's just labeled wrong.

That makes this a **fixable mistake**, not an absent-information case. The move is: retry.

---

### Step 4 — The retry call

This is where the lecture's own example retry text gets reused exactly as written, not paraphrased:

> "Feedback: `desired_action` must be one of `refund` / `replacement` / `store_credit` / `unclear`. You returned `exchange`. Re-extract the return request from the original message. Use `unclear` if the `desired_action` is ambiguous. Do not invent missing information."

That text doesn't go to Claude by itself. Per the "strong retry" structure laid out earlier in this lecture, it gets bundled into one combined follow-up message together with:
1. The original customer message
2. The failed extraction (`desired_action: "exchange"`)
3. The exact validation error above
4. The explicit "do not invent missing information" instruction

Claude isn't told "try again." It's handed the original text, its own wrong answer, the precise rule it broke, and a guardrail against overcorrecting into a guess.

---

### Step 5 — Result: Claude re-extracts

Claude responds again, and this time:

```json
{ "desired_action": "unclear" }
```

Run the same one-line check again:
- `"unclear" not in ("refund", "replacement", "store_credit", "unclear")` evaluates to `False`.
- No error gets appended.

`desired_action` is now valid — not because Claude suddenly figured out exactly what the customer wanted, but because `unclear` is an honest, allowed answer for a genuinely ambiguous request. Schema validation only ever asks "is this value in the list?" — and this time it is. The retry loop's job ends here.

---

### Step 6 — Schema validation passing is not the finish line

This is the part that's easy to miss: fixing the enum doesn't mean the case is ready to automate. The same `validate_return_request` function also checks a completely different thing:
...
And the confidence/signal table earlier in this lecture spells out what that means in practice — "Low confidence on any field" (example: `desired_action_confidence: low`) routes straight to **human review**, same as `conflict_detected: true`. A `desired_action` that only just got resolved to `unclear` after a failed first attempt is exactly the kind of field likely to still carry a low confidence score — the ambiguity that produced the wrong guess the first time doesn't vanish just because the second guess happened to land on a legal word. So even though the retry succeeded, this case can still land in the human review queue — for a completely separate reason than the one that triggered the retry in the first place.

---

### The whole journey, end to end

1. Claude extracts → `desired_action: "exchange"`.
2. Schema validation: `"exchange" not in allowed` → error appended, not automated.
3. Golden rule check: the answer exists in the message, Claude just picked the wrong word for it → fixable → retryable.
4. Retry sent: original message + failed extraction + exact error text + "do not invent missing information."
5. Claude re-extracts → `desired_action: "unclear"` → passes schema validation this time, because `unclear` is on the allowed list.
6. Separately, `desired_action_confidence: low` (or `conflict_detected: true`) can still set `human_review_required` → the case still goes to a person, despite the retry "succeeding."

**The one thing to hold onto:** fixing the *shape* of an answer (the retry) and deciding whether a human needs to see it (the routing) are two separate decisions, made by two separate checks, for two separate reasons — this one example walks through both, back to back, and neither one cancels the other out.
