...
tools.py
---------
The TOOLS our agents can use. Same idea as Demo 1.1: each tool has a SCHEMA
(what the model reads to choose a tool) and a FUNCTION (the code that runs).

NEW IN DEMO 1.2 — tools are grouped by SPECIALIST ROLE.
-------------------------------------------------------
In Demo 1.1 a single agent had ALL the tools. Now we split the work across
specialist subagents, so we also split the tools by role:

  - ORDER tools    -> get_customer, get_order      (the "Order" specialist)
  - POLICY tools   -> get_refund_policy,
                      get_shipping_policy           (the "Policy" specialist)

Giving each subagent only the tools it needs keeps tool selection reliable
(a preview of Task 2.3). The coordinator itself gets NO tools — its only job
is to delegate to subagents and combine their answers.
"""
